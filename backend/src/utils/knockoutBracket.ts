...
export function buildRoundOf32(standings: Record<string, StandingRow[]>): R32Matchup[] {
  ...
  return [
    ...
    m('M81', winner('D'), thirdFor('D'), W('D'), T('D'), '1D vs 3B/E/F/I/J'),
    m('M82', winner('G'), thirdFor('G'), W('G'), T('G'), '1G vs 3A/E/H/I/J'),
    m('M83', runnerUp('K'), runnerUp('L'), R('K'), R('L'), '2K vs 2L'),
    m('M84', winner('H'), runnerUp('J'), W('H'), R('J'), '1H vs 2J'),
    m('M85', winner('B'), thirdFor('B'), W('B'), T('B'), '1B vs 3E/F/G/I/J'),
    m('M86', winner('J'), runnerUp('H'), W('J'), R('H'), '1J vs 2H'),
    m('M87', winner('K'), thirdFor('K'), W('K'), T('K'), '1K vs 3D/E/I/J/L'),
    m('M88', runnerUp('D'), runnerUp('G'), R('D'), R('G'), '2D vs 2G'),
  ]
}
