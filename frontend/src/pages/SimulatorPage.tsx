...
const CACHE_KEY = 'sim-results-v1'
...
export default function SimulatorPage() {
  ...
  useEffect(() => {
    ...
    apiFetch('/bracket/my')
      .then(d => setMyChampion(parseTeamName(d.predictions?.champion?.[0] ?? null)))
      .catch(() => {})
    // Restaurar última corrida de la sesión
    try {
  ...
  const startRun = () => {
    if (!matches) return
    const config: SimConfig = { iterations, surprise, squadWeight, hostBoost, seed }
    run(matches, config)
  }

  const top3 = results ? results.teams.slice(0, 3) : []
  const visibleTeams = results ? (showAll ? results.teams : results.teams.slice(0, 16)) : []
  const myChampionStats = results && myChampion
    ? results.teams.find(t => t.team === myChampion)
    : null
  const myChampionRank = myChampionStats && results
    ? results.teams.indexOf(myChampionStats) + 1
    : null
    ...
    <div className="max-w-7xl mx-auto px-4 md:px-8 py-6 font-sans text-white">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-5">
          ...
          <div>
            ...
            <button onClick={() => setHostBoost(v => !v)} disabled={running}
              ...
              <span className="no-invert">🇲🇽🇺🇸🇨🇦</span> {hostBoost ? 'Activada (+100 Elo)' : 'Desactivada'}
            </button>
          </div>
        </div>
      ...
      {results && (
        <>
          ...
          <div className="bg-panel border border-white/8 rounded-2xl overflow-hidden fade-up-3">
          ...
          <p className="text-[11px] text-gray-600 mt-4 leading-relaxed fade-up-4">
            ...
            eloratings.net y de plantilla por valor de mercado (Transfermarkt, escala log). {results.iterations.toLocaleString('es')} iteraciones
            en {results.elapsedMs.toLocaleString('es')} ms. Los partidos ya jugados quedan fijos con su resultado real.
      ...
      {!results && !running && matches && (
        <div className="bg-panel border border-white/8 rounded-2xl p-10 text-center fade-up-2 relative overflow-hidden">
          ...
          <p className="text-gray-500 text-sm mt-2 max-w-md mx-auto">
            Ajusta las variables y corre miles de mundiales simulados para ver las probabilidades de cada selección.
          </p>
        </div>
      )}
    </div>
